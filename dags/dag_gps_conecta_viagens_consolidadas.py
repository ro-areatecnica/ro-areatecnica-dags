...
def check_failed_endpoints(context=None):
    ...
    query = """
        ...
        FROM `ro-areatecnica.conecta_raw.control_table_viagens_consolidadas`
        WHERE status = 'failed'
    ...
    if failed_endpoints:
        ...
        subject = "FALHA NA TABELA DE CONTROLE DO CONECTA VIAGENS CONSOLIDADAS"
        body = f"""
...
def run_cf():
    ...
    audience = 'https://us-central1-ro-areatecnica.cloudfunctions.net/cf-gps-conecta-viagens_consolidadas'
...
with DAG(
        dag_id="dag_conecta_data_consolidadas",
        default_args=default_args,
        description="Dag que aciona a Cloud Functions cf-gps-conecta-viagens_consolidadas e notifica em caso de falha por email.",
        schedule_interval=timedelta(hours=1),
        catchup=False,
        tags=["conecta_consolidadas"],
) as dag:
